...
namespace {
    ...
    static void manuallyRegisteredTestFunction() { SUCCEED( "was called" ); }
...
TEST_CASE( "Testing checked-if 3", "[checked-if][!shouldfail]" ) {
    CHECKED_ELSE( false ) { FAIL(); }
    // If the checked false is not entered, this passes and the test
    // fails, because of the [!shouldfail] tag.
    SUCCEED();
}

[[noreturn]] TEST_CASE( "Testing checked-if 4", "[checked-if][!shouldfail]" ) {
    CHECKED_ELSE( true ) {}
    throw std::runtime_error( "Uncaught exception should fail!" );
}

[[noreturn]] TEST_CASE( "Testing checked-if 5", "[checked-if][!shouldfail]" ) {
    CHECKED_ELSE( false ) {}
    throw std::runtime_error( "Uncaught exception should fail!" );
}
...
TEMPLATE_TEST_CASE_SIG( "TemplateTestSig: vectors can be sized and resized",
...
TEMPLATE_LIST_TEST_CASE( "Template test case with test types specified inside "
                         "non-default-constructible std::tuple",
                         "[template][list]",
...
struct NonCopyableAndNonMovableType {
    ...
    NonCopyableAndNonMovableType( NonCopyableAndNonMovableType const& ) =
        delete;
...
TEMPLATE_LIST_TEST_CASE( "Template test case with test types specified inside "
                         "non-copyable and non-movable std::tuple",
                         "[template][list]",
...
TEST_CASE( "#835 -- errno should not be touched by Catch2",
           "[.][failing][!shouldfail]" ) {
    errno = 1;
...
TEMPLATE_TEST_CASE_SIG(
    "#1954 - 7 arg template test case sig compiles",
    "[regression][.compilation]",
    ( ( int Tnx, int Tnu, int Tny, int Tph, int Tch, int Tineq, int Teq ),
      Tnx,
      Tnu,
      Tny,
      Tph,
...
TEST_CASE( "Same test name but with different tags is fine",
           "[.approvals][some-tag]" ) {}
TEST_CASE( "Same test name but with different tags is fine",
           "[.approvals][other-tag]" ) {}
...
TEST_CASE( "Validate SEH behavior - handled",
           "[approvals][FatalConditionHandler][CATCH_PLATFORM_WINDOWS]" ) {
    // Validate that Catch2 framework correctly handles tests raising and
    // handling SEH exceptions.
    throw_and_catch();
}

void throw_no_catch() { RaiseException( 0xC0000005, 0, 0, NULL ); }

TEST_CASE( "Validate SEH behavior - unhandled",
           "[.approvals][FatalConditionHandler][CATCH_PLATFORM_WINDOWS]" ) {
    // Validate that Catch2 framework correctly handles tests raising and not
    // handling SEH exceptions.
    throw_no_catch();
}
...
TEST_CASE( "Validate SEH behavior - no crash for stack unwinding",
           "[approvals][!throws][!shouldfail][FatalConditionHandler][CATCH_"
           "PLATFORM_WINDOWS]" ) {
    // Trigger stack unwinding with SEH top-level filter changed and validate
    // the test fails expectedly with no application crash
