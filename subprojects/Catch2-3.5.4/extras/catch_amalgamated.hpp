...
#                if defined( __GNUC__ ) && !defined( __clang__ ) && \
                    !defined( __ICC ) && !defined( __CUDACC__ ) &&  \
...
#                    if !defined( \
                        __clang__ ) // Handle Clang masquerading for msvc
...
#                if defined( CATCH_CONFIG_SHARED_LIBRARY ) && \
                    defined( _MSC_VER ) && !defined( CATCH_CONFIG_STATIC )
#                    ifdef Catch2_EXPORTS
#                        define CATCH_EXPORT //__declspec( dllexport ) // not
                                             //needed
#                    else
...
#                define CATCH_MOVE( ... )                                    \
                    static_cast<                                             \
                        std::remove_reference_t<decltype( __VA_ARGS__ )>&&>( \
                        __VA_ARGS__ )
...
namespace Catch {
    ...
    struct ResultWas {
        enum OfType {
            ...
            Info = 1,
            Warning = 2,
            // TODO: Should explicit skip be considered "not OK" (cf. isOk)?
            // I.e., should it have the failure bit?
            ExplicitSkip = 4,
...
namespace Catch {
    namespace Detail {
        /**
         * A reimplementation of `std::unique_ptr` for improved compilation
         * performance
         *
...
#                pragma warning( push )
#                pragma warning( \
                    disable : 4180 ) // We attempt to stream a function
...
#        define INTERNAL_CATCH_INFO( macroName, log )              \
            const Catch::ScopedMessage INTERNAL_CATCH_UNIQUE_NAME( \
                scopedMessage )(                                   \
...
#        if defined( CATCH_CONFIG_PREFIX_MESSAGES ) && \
...
#                            define CATCH_DEFINE_COMPARABLE_TRAIT( id, op )    \
                                ...
                                struct is_##id##_comparable<                   \
                                    T,                                         \
                                    U,                                         \
                                    void_t<decltype( std::declval<T>() op      \
                                                         std::declval<U>() )>> \
                                    : std::true_type {};                       \
                                template <typename, typename = void>           \
                                struct is_##id##_0_comparable                  \
                                    : std::false_type {};                      \
                                template <typename T>                          \
                                struct is_##id##_0_comparable<                 \
                                    T,                                         \
...
#                            pragma warning( push )
#                            pragma warning( \
                                disable : 4389 ) // '==' : signed/unsigned
                                                 // mismatch
#                            pragma warning( \
                                disable : 4018 ) // more "signed/unsigned
                                                 // mismatch"
#                            pragma warning( \
...
#                        define CATCH_INTERNAL_DEFINE_EXPRESSION_EQUALITY_OPERATOR( \
                            ...
                            constexpr friend auto operator op( ExprLhs&& lhs,       \
                                                               RhsT&& rhs )         \
                                ->std::enable_if_t<                                 \
                                    Detail::conjunction<                            \
                                ...
                                return {                                            \
                                    static_cast<bool>( lhs.m_lhs op rhs ),          \
                                    lhs.m_lhs,                                      \
                                    #op##_sr,                                       \
                                    rhs };                                          \
                            }                                                       \
                            ...
                            constexpr friend auto operator op( ExprLhs&& lhs,       \
                                ...
                                ->std::enable_if_t<                                 \
                                    Detail::conjunction<                            \
                                        Detail::negation<                           \
                                            Detail::is_##id##_comparable<           \
                                                LhsT,                               \
                                                RhsT>>,                             \
                                        Detail::is_eq_0_comparable<                 \
                                            LhsT>, /* We allow long because we      \
                                ...
                                return { static_cast<bool>( lhs.m_lhs op 0 ),       \
                                         lhs.m_lhs,                                 \
                                         #op##_sr,                                  \
                                         rhs };                                     \
                            }                                                       \
                            ...
                            constexpr friend auto operator op( ExprLhs&& lhs,       \
                                ...
                                return { static_cast<bool>( 0 op rhs ),             \
                                         lhs.m_lhs,                                 \
                                         #op##_sr,                                  \
                                         rhs };                                     \
                            }
...
#                        undef CATCH_INTERNAL_DEFINE_EXPRESSION_OPERATOR
    ...
    struct Decomposer {
        ...
        constexpr friend auto operator<=( Decomposer&&, T value )
            -> ExprLhs<T> {
...
#                    define INTERNAL_CATCH_TEST(                                          \
                        ...
                        do { /* NOLINT(bugprone-infinite-loop) */                         \
                            /* The expression should not be evaluated, but                \
                             * warnings should hopefully be checked */                    \
                            ...
                            ( false ) &&                                                  \
                                static_cast<const bool&>(                                 \
                                    !!( __VA_ARGS__ ) ) ) // the expression here
                                                          // is never evaluated
                                                          // at runtime but it
                                                          // forces the compiler
                                                          // to give it a look
...
#                    if defined(                                                      \
                        CATCH_INTERNAL_CONFIG_STATIC_ANALYSIS_SUPPORT ) &&            \
                        !defined(                                                     \
...
#                    define INTERNAL_CATCH_SECTION( ... )                      \
                        ...
                        if ( Catch::Section const& INTERNAL_CATCH_UNIQUE_NAME( \
                                 ...
                                 Catch::Section( CATCH_INTERNAL_LINEINFO,      \
                                                 __VA_ARGS__ ) )               \
                        CATCH_INTERNAL_STOP_WARNINGS_SUPPRESSION
...
#                    define INTERNAL_CATCH_SECTION( ... )                   \
                        ...
                        if ( [[maybe_unused]] const int                     \
                                 catchInternalPreviousSectionHint =         \
                                     catchInternalSectionHint,              \
                             catchInternalSectionHint =                     \
                                 Catch::Detail::GetNewSectionHint(          \
                                     __VA_ARGS__ );                         \
...
#                else // ^^ !CATCH_CONFIG_EXPERIMENTAL_STATIC_ANALYSIS_SUPPORT |
                      // vv CATCH_CONFIG_EXPERIMENTAL_STATIC_ANALYSIS_SUPPORT
...
#                    define INTERNAL_CATCH_TESTCASE2( fname, ... )          \
                        ...
                        CATCH_INTERNAL_SUPPRESS_GLOBALS_WARNINGS            \
                        CATCH_INTERNAL_SUPPRESS_UNUSED_VARIABLE_WARNINGS    \
                        static const Catch::Detail::DummyUse                \
                            INTERNAL_CATCH_UNIQUE_NAME( dummyUser )(        \
                                &( fname ),                                 \
                                Catch::NameAndTags{ __VA_ARGS__ } );        \
                        CATCH_INTERNAL_SUPPRESS_SHADOW_WARNINGS             \
                        static void fname(                                  \
                            [[maybe_unused]] int catchInternalSectionHint ) \
                            CATCH_INTERNAL_STOP_WARNINGS_SUPPRESSION
#                    define INTERNAL_CATCH_TESTCASE( ... )               \
                        INTERNAL_CATCH_TESTCASE2(                        \
                            INTERNAL_CATCH_UNIQUE_NAME( dummyFunction ), \
                            __VA_ARGS__ )
...
#                define INTERNAL_CATCH_TEST_CASE_METHOD2(                     \
                    ...
                    namespace {                                               \
                        ...
                        const Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(      \
                            autoRegistrar )(                                  \
...
#            elif defined( CATCH_CONFIG_PREFIX_ALL ) && \
                defined( CATCH_CONFIG_DISABLE ) // ^^ prefixed, implemented | vv
                                                // prefixed, disabled
...
#            elif !defined( CATCH_CONFIG_PREFIX_ALL ) && \
                !defined( CATCH_CONFIG_DISABLE ) // ^^ prefixed, disabled | vv
                                                 // unprefixed, implemented

#                define REQUIRE( ... )                                     \
                    INTERNAL_CATCH_TEST( "REQUIRE",                        \
                                         Catch::ResultDisposition::Normal, \
                                         __VA_ARGS__ )
...
#            elif !defined( CATCH_CONFIG_PREFIX_ALL ) && \
                defined( CATCH_CONFIG_DISABLE ) // ^^ unprefixed, implemented |
                                                // vv unprefixed, disabled
...
#                    define CATCH_RECURSE( ... ) \
                        CATCH_RECURSION_LEVEL6(  \
                            CATCH_RECURSION_LEVEL6( __VA_ARGS__ ) )
#                else
#                    define CATCH_RECURSE( ... ) \
                        CATCH_RECURSION_LEVEL5( __VA_ARGS__ )
#                endif
...
#                define CATCH_REC_LIST0( f, x, peek, ... ) \
                    , f( x ) CATCH_DEFER( CATCH_REC_NEXT(  \
                          peek, CATCH_REC_LIST1 ) )( f, peek, __VA_ARGS__ )
#                define CATCH_REC_LIST1( f, x, peek, ... ) \
                    , f( x ) CATCH_DEFER( CATCH_REC_NEXT(  \
                          peek, CATCH_REC_LIST0 ) )( f, peek, __VA_ARGS__ )
...
#                define CATCH_REC_LIST0_UD( f, userdata, x, peek, ... ) \
                    , f( userdata, x ) CATCH_DEFER(                     \
                          CATCH_REC_NEXT( peek, CATCH_REC_LIST1_UD ) )( \
                          f, userdata, peek, __VA_ARGS__ )
#                define CATCH_REC_LIST1_UD( f, userdata, x, peek, ... ) \
                    , f( userdata, x ) CATCH_DEFER(                     \
                          CATCH_REC_NEXT( peek, CATCH_REC_LIST0_UD ) )( \
                          f, userdata, peek, __VA_ARGS__ )
...
#                define CATCH_REC_LIST_UD( f, userdata, ... )       \
                    CATCH_RECURSE( CATCH_REC_LIST2_UD( f,           \
                                                       userdata,    \
                                                       __VA_ARGS__, \
                                                       ()()(),      \
                                                       ()()(),      \
...
#                define INTERNAL_CATCH_TYPE_GEN                               \
                    ...
                    constexpr auto get_wrapper() noexcept                     \
                        ->TemplateTypeList<Cs...> {                           \
                        return {};                                            \
                    }                                                         \
                    template <typename...>                                    \
                    ...
                    struct append<L1<E1...>, L2<E2...>, Rest...> {            \
                        using type =                                          \
                            typename append<L1<E1..., E2...>, Rest...>::type; \
                    };                                                        \
                    template <template <typename...> class L1,                \
                              typename... E1,                                 \
                              typename... Rest>                               \
                    ...
                    struct convert<Final, List<Ts...>> {                      \
                        using type =                                          \
                            typename append<Final<>, TypeList<Ts>...>::type;  \
                    };
...
#            define INTERNAL_CATCH_TEMPLATE_TEST_CASE_2(                                                                                             \
                ...
                namespace {                                                                                                                          \
                    namespace INTERNAL_CATCH_MAKE_NAMESPACE( TestName ) {                                                                            \
                        ...
                        static const int INTERNAL_CATCH_UNIQUE_NAME(                                                                                 \
                            globalRegistrar ) = []() {                                                                                               \
                            TestName<                                                                                                                \
...
#                    undef SizedUnsignedTypeHelper
         ...
         *
         * This implementation is almost twice as fast as naive long
         * multiplication, and unlike intrinsic-based approach, it supports
...
namespace Catch {
    ...
    class uniform_integer_distribution {
        ...
        // How many different values are there in [a, b]. a == b => 1, can be 0
        // for distribution over all values in the type.
        ...
        // distributions only to implement random generators, we assume that
        // each distribution will be reused many times and this is an
        // optimization.
        UnsignedIntegerType m_rejection_threshold = 0;

        UnsignedIntegerType computeDistance( IntegerType a,
                                             IntegerType b ) const {
    ...
    public:
        ...
        uniform_integer_distribution( IntegerType a, IntegerType b ):
            ...
            m_rejection_threshold(
                computeRejectionThreshold( m_ab_distance ) ) {
            assert( a <= b );
        }
        ...
        result_type operator()( Generator& g ) {
            ...
            if ( m_ab_distance == 0 ) {
                return transposeBack(
                    Detail::fillBitsFrom<UnsignedIntegerType>( g ) );
            }
...
namespace Catch {
    ...
    namespace Detail {
        /**
         * Returns the largest magnitude of 1-ULP distance inside the [a, b]
         * range.
         *
         * Assumes `a < b`.
         */
        ...
        FloatType gamma( FloatType a, FloatType b ) {
            ...
            const auto gamma_down =
                b - Catch::nextafter(
                        b, -std::numeric_limits<FloatType>::infinity() );
...
#                    endif
        ...
        DistanceType<FloatType> count_equidistant_floats( FloatType a,
            ...
            const auto err = ( std::fabs( a ) <= std::fabs( b ) )
                                 ...
                                 : bg - ( s + ag );
            const auto ceil_s =
                static_cast<DistanceType<FloatType>>( std::ceil( s ) );
...
#                endif
    ...
    class uniform_floating_point_distribution {
        ...
        static_assert( !std::is_same<FloatType, long double>::value,
                       "We do not support long double due to inconsistent "
...
namespace Catch {
    ...
    /**
     * Helper for streaming a "count [maybe-plural-of-label]" human-friendly
     * string
     *
     * Usage example:
     * ```cpp
    ...
    namespace Matchers {
        ...
        namespace Detail {
            ...
            public:
                ...
                friend MatchAllOf operator&&( MatcherBase<ArgT> const& lhs,
                                              MatchAllOf&& rhs ) {
            ...
            template <typename ArgT>
            MatchAllOf<ArgT>
            operator&&( MatchAllOf<ArgT> const& lhs,
            ...
            public:
                ...
                friend MatchAnyOf operator||( MatcherBase<ArgT> const& lhs,
                                              MatchAnyOf&& rhs ) {
                    rhs.m_matchers.insert( rhs.m_matchers.begin(), &lhs );
                    return CATCH_MOVE( rhs );
                }
            ...
            template <typename ArgT>
            MatchAnyOf<ArgT>
            operator||( MatchAnyOf<ArgT> const& lhs,
                        MatcherBase<ArgT> const& rhs ) = delete;
            //! lvalue overload is intentionally deleted, users should
            //! not be trying to compose stored composition matchers
            template <typename ArgT>
...
namespace Catch {
    namespace Matchers {
        ...
        /**
         * Creates a matcher that calls delegates `match` to the provided
         * predicate.
         *
        ...
        Predicate( Pred&& predicate, std::string const& description = "" ) {
            ...
            static_assert(
                std::is_same<bool, FunctionReturnType<Pred, T>>::value,
                "Predicate does not return bool" );
            return PredicateMatcher<T, Pred>( CATCH_FORWARD( predicate ),
                                              description );
        }
