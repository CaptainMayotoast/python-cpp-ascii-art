...
#define PNG_CLEANUP                                                                                     \
    if (png_handler.png_ptr) {                                                                          \
        if (png_handler.row_ptr) png_free(png_handler.png_ptr, png_handler.row_ptr);                    \
...
void
user_read_data(png_structp png_ptr, png_bytep data, size_t length)
{
...
void*
limited_malloc(png_structp, png_alloc_size_t size)
{
...
void
default_free(png_structp, png_voidp ptr)
{
    return free(ptr);
}
...
LLVMFuzzerTestOneInput(const uint8_t* data, size_t size)
{
    ...
    png_handler.row_ptr =
            png_malloc(png_handler.png_ptr, png_get_rowbytes(png_handler.png_ptr, png_handler.info_ptr));
