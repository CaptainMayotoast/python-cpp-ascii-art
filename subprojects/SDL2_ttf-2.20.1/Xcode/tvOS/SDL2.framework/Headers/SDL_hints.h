...
/**
 *  \brief If set to "0" then never set the top most bit on a SDL Window, even if the video mode expects
 * it. This is a debugging aid for developers and not expected to be used by end users. The default is
 * "1"
 *
 *  This variable can be set to the following values:
 ...
 *  This variable can be set to the following values:
 *
 *    "ambient"     - Use the AVAudioSessionCategoryAmbient audio category, will be muted by the phone
 * mute switch (default) "playback"    - Use the AVAudioSessionCategoryPlayback category
 *
...
/**
 *  \brief  A variable that controls whether Steam Controllers should be exposed using the SDL joystick
 ...
 *    "X"       - Enable 3D acceleration, using X where X is one of the valid rendering drivers.  (e.g.
 * "direct3d", "opengl", etc.)
 *
...
/**
 *  \brief  If set, all devices will be skipped when scanning for game controllers except for the ones
...
/**
 *  \brief  If set, game controller face buttons report their values according to their labels instead of
 * their positional layout.
 *
...
/**
 * \brief A variable to control whether certain IMEs should handle text editing internally instead of
 * sending SDL_TEXTEDITING events.
 *
 * The variable can be set to the following values:
 ...
 *    "1"       - The indicator bar is hidden and is shown when the screen is touched (useful for movie
 * playback applications) "2"       - The indicator bar is dim and the first swipe makes it visible and
 * the second swipe performs the "home" action (default for fullscreen applications)
 */
#    define SDL_HINT_IOS_HIDE_HOME_INDICATOR "SDL_IOS_HIDE_HOME_INDICATOR"

/**
 *  \brief  A variable that lets you enable joystick (and gamecontroller) events even when your app is in
 * the background.
 *
 *  The variable can be set to the following values:
...
/**
 *  \brief  A variable controlling whether the HIDAPI driver for Nintendo GameCube controllers should be
 * used.
 *
 *  This variable can be set to the following values:
...
/**
 *  \brief  A variable controlling whether "low_frequency_rumble" and "high_frequency_rumble" is used to
 * implement the GameCube controller's 3 rumble modes, Stop(0), Rumble(1), and StopHard(2) this is useful
 * for applications that need full compatibility for things like ADSR envelopes. Stop is implemented by
 * setting "low_frequency_rumble" to "0" and "high_frequency_rumble" ">0" Rumble is both at any arbitrary
 * value, StopHard is implemented by setting both "low_frequency_rumble" and "high_frequency_rumble" to
 * "0"
 *
 *  This variable can be set to the following values:
...
/**
 *  \brief  A variable controlling whether Switch Joy-Cons should be treated the same as Switch Pro
...
/**
 *  \brief  A variable controlling whether the HIDAPI driver for Amazon Luna controllers connected via
 * Bluetooth should be used.
 *
 *  This variable can be set to the following values:
 *    "0"       - HIDAPI driver is not used
 *    "1"       - HIDAPI driver is used
...
/**
 *  \brief  A variable controlling whether extended input reports should be used for PS4 controllers when
...
/**
 *  \brief  A variable controlling whether extended input reports should be used for PS5 controllers when
 * using the HIDAPI driver.
 *
 *  This variable can be set to the following values:
 *    "0"       - extended reports are not enabled (the default)
...
/**
 *  \brief  A variable controlling whether the HIDAPI driver for Nintendo Switch controllers should be
 * used.
 *
 *  This variable can be set to the following values:
 *    "0"       - HIDAPI driver is not used
...
/**
 *  \brief  A variable controlling whether the Home button LED should be turned on when a Nintendo Switch
...
/**
 *  \brief  A variable controlling whether to use the classic /dev/input/js* joystick interface or the
 * newer /dev/input/event* joystick interface on Linux
 *
 *  This variable can be set to the following values:
...
/**
 *  \brief  A variable controlling whether joysticks on Linux adhere to their HID-defined deadzones or
...
/**
 *  \brief  A variable setting the speed scale for mouse motion, in floating point, when the mouse is not
 * in relative mode
 */
#    define SDL_HINT_MOUSE_NORMAL_SPEED_SCALE "SDL_MOUSE_NORMAL_SPEED_SCALE"

/**
 *  \brief  A variable controlling whether relative mouse mode constrains the mouse to the center of the
 * window
 *
 *  This variable can be set to the following values:
 ...
 *    "1"       - Mouse events will generate touch events (default for mobile platforms, such as Android
 * and iOS)
 */
 ...
 *    "2"     - Use the driver line API (occasionally misses line endpoints based on hardware driver
 * quirks, was the default before 2.0.20) "3"     - Use the driver geometry API (correct, draws thicker
 * diagonal lines)
 *
 ...
 *    "1" or "overscan"  - Will zoom the rendering so it fills the entire screen, allowing edges to be
 * drawn offscreen
 *
...
/**
 *  \brief  A variable controlling whether updates to the SDL screen surface should be synchronized with
 ...
 *
 * The variable can be set to the following values:
 *   "0"       - The return key will be handled as a key event. This is the behaviour of SDL <= 2.0.3.
 ...
 *  provides a relatively small default thread stack size (a few kilobytes versus the default 8MB glibc
 * uses). Support for this hint is currently available only in the pthread, Windows, and PSP backend.
 *
 ...
 *
 *  This variable can be set to the following values:
 *    "0"       - Use Atomics and WaitOnAddress API when available. If not, fall back to Kernel Objects.
...
/**
 *  \brief  A variable controlling whether the window frame and title bar are interactive when the cursor
 * is hidden
 *
 *  This variable can be set to the following values:
...
/**
 *  \brief  A variable controlling whether the window is activated when the SDL_ShowWindow function is
 * called
 *
 *  This variable can be set to the following values:
 ...
 */
typedef void(SDLCALL* SDL_HintCallback)(
        void* userdata,
        const char* name,
        const char* oldValue,
        const char* newValue);
