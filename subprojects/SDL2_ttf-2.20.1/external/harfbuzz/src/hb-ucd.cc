...
static inline bool
_hb_ucd_decompose_hangul (hb_codepoint_t ab,
			  hb_codepoint_t *a,
			  hb_codepoint_t *b)
{
  unsigned si = ab - SBASE;

  if (si >= SCOUNT) return false;
...
_hb_ucd_compose_hangul (hb_codepoint_t a, hb_codepoint_t b, hb_codepoint_t *ab)
{
  ...
  else if (a >= LBASE && a < (LBASE + LCOUNT) && b >= VBASE &&
	   b < (VBASE + VCOUNT))
  {
    /* L,V */
    int li = a - LBASE;
...
_cmp_pair (const void *_key, const void *_item)
{
  ...
  uint64_t b =
      (*(uint64_t *) _item) & HB_CODEPOINT_ENCODE3 (0x1FFFFFu, 0x1FFFFFu, 0);
...
_cmp_pair_11_7_14 (const void *_key, const void *_item)
{
  ...
  uint32_t b = (*(uint32_t *) _item) &
	       HB_CODEPOINT_ENCODE3_11_7_14 (0x1FFFFFu, 0x1FFFFFu, 0);
		...
		void *user_data HB_UNUSED)
{
  ...
  if ((a & 0xFFFFF800u) == 0x0000u && (b & 0xFFFFFF80) == 0x0300u)
  {
    ...
    const uint32_t *v =
	hb_bsearch (k, _hb_ucd_dm2_u32_map, ARRAY_LENGTH (_hb_ucd_dm2_u32_map),
		    sizeof (*_hb_ucd_dm2_u32_map), _cmp_pair_11_7_14);
  ...
  else
  {
    ...
    const uint64_t *v =
	hb_bsearch (k, _hb_ucd_dm2_u64_map, ARRAY_LENGTH (_hb_ucd_dm2_u64_map),
		    sizeof (*_hb_ucd_dm2_u64_map), _cmp_pair);
    if (likely (!v)) return false;
...
static struct hb_ucd_unicode_funcs_lazy_loader_t
    : hb_unicode_funcs_lazy_loader_t<hb_ucd_unicode_funcs_lazy_loader_t>
{
  static hb_unicode_funcs_t *create ()
  {
    hb_unicode_funcs_t *funcs = hb_unicode_funcs_create (nullptr);

    hb_unicode_funcs_set_combining_class_func (funcs, hb_ucd_combining_class,
					       nullptr, nullptr);
    hb_unicode_funcs_set_general_category_func (funcs, hb_ucd_general_category,
						nullptr, nullptr);
    hb_unicode_funcs_set_mirroring_func (funcs, hb_ucd_mirroring, nullptr,
					 nullptr);
    ...
    hb_unicode_funcs_set_decompose_func (funcs, hb_ucd_decompose, nullptr,
					 nullptr);
