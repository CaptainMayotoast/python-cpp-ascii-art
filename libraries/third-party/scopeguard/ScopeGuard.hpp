...
#define finally                                                                                         \
    auto SG_LINE_NAME(ScopeGuard_Block_, __LINE__) = sg::detail::eSgFinally() + [&]() noexcept -> void
#define INVOKE_ON_EXIT(callback) auto SG_LINE_NAME(ScopeGuard_, __LINE__) = sg::MakeScopeGuard(callback)
...
constexpr bool is_proper_callback_v =
        std::is_same<void, decltype(std::declval<T>()())>::value;  // std::is_same_v is not defined until
                                                                   // C++17

template<typename TCallback, typename = std::enable_if_t<is_proper_callback_v<TCallback>>>
class ScopeGuard;
...
template<typename TCallback>
constexpr ScopeGuard<TCallback>
MakeScopeGuard(TCallback&& callback)
{
    return ScopeGuard<TCallback>(std::forward<TCallback>(callback));
}
/* ----------------------------------------------------- */
...
class ScopeGuard<TCallback> final
{
  ...
  public:
    ...
    ScopeGuard(ScopeGuard&& other) noexcept(
            std::is_nothrow_move_constructible<Callback>::value)  // std::is_nothrow_move_constructible_v
...
/* --- The helper functions provided multiple usages --- */
inline ScopeGuard
operator+(eSgFinally, std::function<void()> callback)
{
    return ScopeGuard(callback);
}

inline ScopeGuard
MakeScopeGuard(std::function<void()> callback)
{
    return ScopeGuard(callback);
}
/* ----------------------------------------------------- */
