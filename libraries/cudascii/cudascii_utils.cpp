...
ascii_char_to_patch(char* character, int patch_width, int patch_height)
{
    ...
    SDL_Surface* textSurface = SDL_ConvertSurfaceFormat(
            TTF_RenderText_Shaded(font, character, foregroundColor, backgroundColor),
            SDL_PixelFormatEnum::SDL_PIXELFORMAT_RGBA8888,
            0);

    for (int row = 0; row < textSurface->h; row++) {
        for (int col = 0; col < textSurface->w; col++) {
            const auto value = get_pixel(textSurface, row, col).value_or(0);
...
ascii_chars_to_patchs(std::string_view chars, int patch_width, int patch_height)
{
    ...
    std::ranges::transform(chars, std::back_inserter(pairs), [patch_width, patch_height](char c) {
        return ascii_char_to_patch(&c, patch_width, patch_height);
    });
    return pairs;
}
