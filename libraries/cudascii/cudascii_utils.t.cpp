...
/// @brief a front size of height 14
const constexpr int FontSize_14{14};

/// @brief the max size in pixels that SDL will generate from a given ASCII character
const constexpr std::size_t MaxSizePixels_font14{10U * 16U};

/// @brief the min size in pixels that SDL will generate from a given ASCII character
const constexpr std::size_t MinSizePixels_font14{8U * 16U};

/// @brief lower case 'm' in pixels (generated by SDL)
/// @note the following vectors assume 14 point font, 10px wide, 16 tall (14px + 1px padding + 1px
/// padding)
...
/// @brief a ' ' (space) in pixels (generated by SDL)
/// @note the following vectors assume 14 point font, 8px wide, 16 tall (14px + 1px padding + 1px
/// padding)
...
/// @brief a '~' in pixels (generated by SDL)
/// @note the following vectors assume 14 point font, 8px wide, 16 tall (14px + 1px padding + 1px
/// padding)
...
TEST_CASE("ascii_char_to_patch_m_font_14", "[cudascii_utils]")
{
    std::string_view m{"m"};

    const auto calculated_m = cudascii::utils::ascii_char_to_patch(m, FontSize_14);

    CHECK(calculated_m.size() == MaxSizePixels_font14);
    REQUIRE(Actual_m == calculated_m);

    ankerl::nanobench::Bench().run("ascii_char_to_patch", [&]() {
        ankerl::nanobench::doNotOptimizeAway(cudascii::utils::ascii_char_to_patch(m, FontSize_14));
    });
}

TEST_CASE("ascii_chars_to_patchs_font_14", "[cudascii_utils]")
{
    std::string_view characters{"Nm45"};

    const auto calculated_patches = cudascii::utils::ascii_chars_to_patchs(characters, FontSize_14);

    CHECK(calculated_patches.size() == characters.size());
    CHECK(calculated_patches.at(characters.find('m')).size() == MaxSizePixels_font14);
    REQUIRE(calculated_patches.at(characters.find('m')) == Actual_m);

    ankerl::nanobench::Bench().run("ascii_chars_to_patchs", [&]() {
        ankerl::nanobench::doNotOptimizeAway(
                cudascii::utils::ascii_chars_to_patchs(characters, FontSize_14));
    });
}
...
TEST_CASE("ascii_chars_to_patchs_front_back_font_14", "[cudascii_utils]")
{
    ...
    const auto calculated_patches =
            cudascii::utils::ascii_chars_to_patchs(sv_ascii_characters, FontSize_14);

    // Space
    CHECK(calculated_patches.size() == sv_ascii_characters.size());
    CHECK(calculated_patches.at(sv_ascii_characters.find(' ')).size() == MinSizePixels_font14);
...
TEST_CASE("ascii_chars_to_patchs_sizes_font_14", "[cudascii_utils]")
{
    ...
    // 128
    CHECK(*min == MinSizePixels_font14);
